...
int run(char **tokens, int** status){
	pid_t pid;
//	int child_status;
	pid = fork(); 
        if (pid == 0) { // child
        	...
        	(void) wait(*status); // block until child terminates /
...
//int child_status;
		*status = fly(tokens, back_pid, &num_backPids, &status, back_type, &num_backType);
	}//end else if
...
//*************************************************************************************************
int delta (int who, pid_t *back_pid, int *num_backPids){
	int index, position;
...
pid_t *delta_2(int position, pid_t *back_pid, int *num_backPids){
	(*num_backPids)--;
...
char **delta_3(int position, char **back_type, int *num_backType){
	(*num_backType)--;
...
//*****************************************************************************************************************
    ...
    //                            printf("pid %d has exited\n", who);
				int position = delta (who, back_pid, &num_backPids);
				...
				back_pid = delta_2 (position, back_pid, &num_backPids);
				back_type = delta_3 (position, back_type, &num_backType);
...
//********************************************************************************************************************
			...
			if (fgets(user_answer, SIZE, stdin) == '\0'){ //if user enters ctl-D, exit with status 0
				...
				parse(tokens, num_tokens, &num_vars, &showTokens, var_names, var_vals, prompt, back_pid, &num_backPids, &status, back_type, &num_backType); //validate correctness of user_input and call/execute commands appropriately
				printf("NV1: %i\n", num_vars);
		//		for(int i=0; i<num_vars; i++){
		//		printf("G: %s\n", var_names[i]);
		//		}
